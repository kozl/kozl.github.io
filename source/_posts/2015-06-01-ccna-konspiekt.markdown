...
- [7.0 Troubleshooting](#troubleshooting)
	- [7.2 Troubleshoot and resolve VLAN problems](#troubleshoot-and-resolve-vlan-problems)
	- [7.3 Troubleshoot and resolve trunking problems on Cisco switches](#troubleshoot-and-resolve-trunking-problems-on-cisco-switches)
	- [7.4 Troubleshoot and resolve ACL issues](#troubleshoot-and-resolve-acl-issues)
	- [7.5 Troubleshoot and resolve Layer 1 problems](#troubleshoot-and-resolve-layer-1-problems)
	- [7.7 Troubleshoot and resolve spanning tree operation issues](#troubleshoot-and-resolve-spanning-tree-operation-issues)
	- [7.8 Troubleshoot and resolve routing issues](#troubleshoot-and-resolve-routing-issues)
	- [7.9 Troubleshoot and resolve OSPF problems](#troubleshoot-and-resolve-ospf-problems)
	- [7.10 Troubleshoot and resolve EIGRP problems](#troubleshoot-and-resolve-eigrp-problems)
	- [7.11 Troubleshoot and resolve interVLAN routing problems](#troubleshoot-and-resolve-intervlan-routing-problems)
	- [7.13 Monitor NetFlow statistics](#monitor-netflow-statistics)
	- [7.14 TS EtherChannel problems](#ts-etherchannel-problems)
...
2. Настройка при помощи протокола LACP - IEEE 802.3ad
3. Настройка при помощи протокола PagP - проприетарный протокол

2 и 3 используются для автоматического создания аггрегированного канала
...
Метод генерации младших 64 бит адреса, используется MAC адрес, между старшими и младшими 3 байтами которого вставляется последовательность FF FE. При этом нужно инвертировать 7 бит MAC адреса.
...
0x2102 - поиск начальной конфигурации в NVRAM
0x2142 - игнорировать начальную конфигурацию в NVRAM
...
**Feasible Distance** - Administrative distance + стоимость пути до маршрутизатора, анонсировавшего этот маршрут

**Feasible Successors** - резервные маршруты до сети назначения

**Feasibility condition** - условие, при котором маршрут становится FS, его AD должна быть меньше FD текцщего Successor'a
...
**Administrative distance** - приоритет маршрутов, полученных от данного протокола маршрутизации, чем он меньше, тем лучшим считается маршрут.У EIGRP AD=90

```
router eigrp 65001
 distance eigrp 60 120
```

**Metric composition** - принцип расчёта метрики, по которым выбирается наилучший маршрут. Для расчёта метрики IEGRP использует 5 параметров, которые позволяют выбрать вес для следующих характеристик каналов: пропускная способность, задержка, надежность, загрузка канала. По умолчанию метрика пропорциональна сумме пропускной способности и задержки. Bandwith маршрута равна наименьшей bandwith составляющих его каналов.
...
**Router ID** - уникальный в пределах AS идентификатор маршрутазатора
...
**Auto summary** - автосуммирование анонсированных маршрутов до классовых сетей
...
**Path selection** - наилучшим считается маршрут с наименьшей композитной метрикой.

**Load balancing** - нагрузка балансируется для маршрутов с одинаковой метрикой. Возможна балансировка по маршрутам с разной метрикой между Successor и Fesible successor, если настроить variance, множителя, на который умножается FD лучшего маршрута для определения feasible маршрутов, по которым будет балансироваться нагрузка.
...
#### 5.3 Configure and verify ACLs in a network enviroment

**Именованый**

```
R1(conf)# ip access-list standard <NAME>
R1(conf-std-nacl)# deny 10.0.0.0 0.0.0.255
```

**Нумерованный**

```
R1(conf)# access-list <1-99> deny 10.0.0.0 0.0.0.255
```

**Применение ACL на интерфейсе**

```
R1(conf-if)# ip access-group <1-99> <in | out>
```

**Применение ACL на vty**
...
SW(config)# ip flow-export destination <IP> <PORT>
SW(config)# ip flow-export version <VERSION>
```

#### 7.14 TS EtherChannel problems

Если не используется ни один из протоколов согласования параметров аггрегированного канала, нужно иметь ввиду, что на обоих концах канала следующие параметры портов должны совпадать:
